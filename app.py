...
from alpaca.trading.requests import GetOrdersRequest, MarketOrderRequest, TrailingStopOrderRequest, StopOrderRequest
...
ENTRY_FILL_POLL_ATTEMPTS = int(os.environ.get("ENTRY_FILL_POLL_ATTEMPTS", "60"))
ENTRY_FILL_POLL_SECONDS = float(os.environ.get("ENTRY_FILL_POLL_SECONDS", "0.15"))
    ...
    try:
        with sqlite3.connect(STATE_DB_PATH) as conn:
            ...
            for symbol, count in conn.execute(
                "SELECT symbol, loss_count FROM daily_losses WHERE trade_date = ?", (today_str,)
            ):
                daily_loss_counts[symbol] = count
            conn.execute("DELETE FROM daily_losses WHERE trade_date < ?", (today_str,))

        logger.info(
            f"Loaded state: active_trades={len(active_trades)}, "
            f"processed_alerts={len(processed_alerts)}, "
            f"daily_losses={daily_loss_counts}"
        )
...
def record_symbol_loss(symbol):
    ...
    with sqlite3.connect(STATE_DB_PATH) as conn:
        conn.execute(
            "INSERT OR REPLACE INTO daily_losses(symbol, trade_date, loss_count) VALUES (?, ?, ?)",
            (symbol, today_str, daily_loss_counts[symbol]),
        )
...
async def process_symbol_alert(symbol, side, payload, background_tasks):
    ...
    logger.info(
        ...
        f"(time_scale={time_scale:.2f}, loss_decay={loss_decay:.2f}, losses_today={losses_today}), "
        f"trail_atr_multiple={trade_params['trail_atr_multiple']:.2f}"
    )

    parallel_tasks = [
        alpaca_call(client.get_all_positions),
        get_open_symbol_orders(client, symbol),
        alpaca_call(client.get_account),
        latest_trade_price(symbol),
    ]
    if side == "sell":
        parallel_tasks.append(alpaca_call(client.get_asset, symbol))

    results = await asyncio.gather(*parallel_tasks, return_exceptions=True)
    positions = results[0]
    open_orders = results[1]
    account = results[2]
    market_price_result = results[3]
    asset_result = results[4] if side == "sell" else None

    for i, r in enumerate(results):
        if isinstance(r, Exception):
            logger.error(f"Parallel validation call {i} failed: {r}")
            raise HTTPException(status_code=503, detail=f"Pre-trade validation failed: {str(r)}")
    ...
    if side == "sell":
        if not getattr(asset_result, "shortable", False):
            raise HTTPException(status_code=409, detail=f"{symbol} is not shortable.")

    market_price = float(market_price_result)
    if alert_price is not None:
        slippage_bps = abs(market_price - alert_price) / alert_price * 10000
        if slippage_bps > MAX_SLIPPAGE_BPS:
            raise HTTPException(
                status_code=409,
                detail=f"Slippage too high: alert={alert_price:.2f}, market={market_price:.2f}, bps={slippage_bps:.1f}",
            )
    else:
        slippage_bps = 0.0
    ...
    async with EXECUTION_LOCK:
        try:
            ...
            for pos in open_positions:
                ...
                logger.info(
                    f"Heartbeat: {symbol} side={position_side} protected by order {protection_order.id}. "
                    f"entry={entry_price:.4f}, current={current_price:.4f}, "
                    f"trail_price={trail_price:.2f}, favorable_move={favorable_move:.2f}"
                )

                # --- NEW HARD BREAKEVEN STOP LOGIC ---
                if not trade.get("breakeven_stop_applied") and favorable_move >= BREAKEVEN_ATR_TRIGGER * atr:
                    try:
                        ...
                        logger.info(
                            ...
                            f"Replaced trailing stop with hard breakeven stop at {entry_price:.2f}. new_order={new_trail.id}"
                        )
