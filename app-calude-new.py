...
TRAIL_PRICE_ATR_MULTIPLE = float(os.environ.get("TRAIL_PRICE_ATR_MULTIPLE", "1.0"))
PARTIAL_TAKE_PROFIT_ENABLED = os.environ.get("PARTIAL_TAKE_PROFIT_ENABLED", "true").lower() == "true"
PARTIAL_TAKE_PROFIT_TRIGGER_ATR = float(os.environ.get("PARTIAL_TAKE_PROFIT_TRIGGER_ATR", "1.0"))
...
app = FastAPI(title="9:30 AM Breakout Execution Engine v7.0-CleanHybrid", lifespan=lifespan)
...
def calculate_position_size(account, market_price, trail_price):
    buying_power = float(account.daytrading_buying_power)
    allocation_cash = min(round(buying_power / 3, 2), MAX_TRADE_ALLOCATION)
...
async def take_partial_profit_and_rearm(client, pos, trade, protection_order):
    ...
    partial_qty = int(current_qty * PARTIAL_TAKE_PROFIT_FRACTION)
...
async def process_symbol_alert(symbol, side, payload, background_tasks):
    ...
    trail_price = round(atr_value * TRAIL_PRICE_ATR_MULTIPLE, 2)
    if trail_price <= 0:
    ...
    try:
        ...
        background_tasks.add_task(send_sms_alert_sync, "STRATEGY ENTRY", f"{symbol} {side.upper()} filled. ATR trailing stop armed.")
        return {
            "status": "success",
        ...
        try:
            ...
            for symbol in list(active_trades):
                if symbol not in open_symbols:
                    logger.info(f"Removing inactive trade tracking for {symbol}.")
                    remove_active_trade(symbol)
            ...
            for pos in open_positions:
                ...
                logger.info(
                    ...
                    f"trail_price={float(trade['trail_price']):.2f}"
                )
                ...
                trigger_move = PARTIAL_TAKE_PROFIT_TRIGGER_ATR * atr
